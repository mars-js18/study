...
function switchTab(tabId) {
  ...
  // Update Nav Buttons Styles (Sidebar & Mobile Bottom Nav)
  document.querySelectorAll('.nav-tab').forEach(btn => {
    const isCurrent = btn.dataset.tab === tabId;
    if (btn.classList.contains('mobile-bottom-tab')) {
      if (isCurrent) {
        btn.className = 'nav-tab mobile-bottom-tab flex-1 flex flex-col items-center justify-center py-1 text-indigo-400 font-semibold transition-all';
      } else {
        btn.className = 'nav-tab mobile-bottom-tab flex-1 flex flex-col items-center justify-center py-1 text-slate-400 hover:text-slate-200 transition-all';
      }
    } else {
      if (isCurrent) {
  ...
  // Scroll main view container to top smoothly on mobile/desktop
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const btnOpenBackupMobile = document.getElementById('btn-open-backup-mobile');
  if (btnOpenBackupMobile) {
    btnOpenBackupMobile.addEventListener('click', () => {
      closeMobileDrawer();
